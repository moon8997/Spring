...
select seq, fk_userid, name, subject, content, readCount
    , to_char(regDate, 'yyyy-mm-dd hh24:mi:ss') as regDate
from tbl_board
where status = 1 and seq = 'sdfsdfddsfㄴㄹㄴㅇㄹㅇㄴㄹㄴ';
...
order by pk_seq desc;
 ...
 insert into tbl_board(pk_seq, header, fk_emp_no, user_name, subject, content, pw, read_count, write_day, status, groupno, fk_seq, depthno)
...
from tbl_board
where status = 1
and lower(ㄴㄹㄹㅇㄹㅇㄴㄹ) like '%'|| lower('J') ||'%';


select count(*)
from tbl_board
where status = 1
and lower(subject) like '%'|| lower('ㄴㄹㅇㄴㄹㄴㄹㅇㄴㄹ') ||'%';
...
select userid, email
from tbl_member
where userid in('tjdansrlf','leess');

update tbl_member set email = (select email from tbl_member where userid = 'tjdansrlf')
where userid = 'leess';
...
values(seq_reservation.nextval, 'tjdansrlf', to_date('2022-05-12 13:00','yyyy-mm-dd hh24:mi') );

insert into tbl_reservation(reservationSeq, fk_userid, reservationDate)
values(seq_reservation.nextval, 'leess', to_date('2022-05-12 14:00','yyyy-mm-dd hh24:mi') );

insert into tbl_reservation(reservationSeq, fk_userid, reservationDate)
values(seq_reservation.nextval, 'tjdansrlf', to_date('2022-05-13 11:00','yyyy-mm-dd hh24:mi') );
